...
5. **Broken correlations**: In human data, mean_acceleration and mean_jerk are uncorrelated (r=-0.025). In all synthetic approaches, they're near-perfectly correlated (r=0.999). The RF detects this joint distribution mismatch, not individual feature gaps.
   ...
   **AMENDED 2026-07-27, do not quote the numbers above.** The r=-0.025 figure is Pearson on a sample that cannot be identified and is not reproducible as stated. These two features carry extreme outliers, mean_acceleration spans -2.3e5 to 5.1e7 on the human reference, so Pearson here is set by a handful of paths: the human pair reads 0.9999 on Pearson, 0.193 on Spearman, and -0.138 inside the slowest decile. Note also that line 1358 of this file records the human pair as **+1.000**, the opposite sign to line 177, which is the same instability. On rank correlation on the discrete event-stream model the pair is arm +0.140 against human +0.193, ie. not a defect. The headline claim that the RF is reading joint structure rather than marginals **does survive**, and was re-measured properly on 2026-07-27 (`research/w3_joint_structure.py`, `research/w3_coupling_gate.py`): forcing all 18 marginals to human removes only 32% to 37% of the gap. But forcing every pairwise correlation to human as well removes nothing further beyond that, so the joint structure the RF is reading is **higher-order, not pairwise**, and this pair is not the handle. See HANDOFF.md "What the gap is made of".
