"""w4_durmech, AMENDMENT 20 in step0_prereg.md. Read that first.

Which ingredient of the matched draw unlocks q. Per seed: one spec set;
THREE log duration vectors drawn once each: M = durmatch matcher verbatim
(rng 9000+seed); MJ = the same M values plus the DurationModel's own
jitter, normal(0, 0.02*std_mult 1.0) in log space, rng 17000+seed, then
the same clip through exp to [0.05, 4.0] and back; P = protocol draw
(esp._duration.sample). SIX arms on identical specs: {M, MJ, P} x {k0,
q1}, full q from training/w4_firsthead_q.pt, confirm per batch torch
seeding, contract scorer after shuffle. Seeds 30 to 34, n 2000.

THIS ARM PRODUCES NO HEADLINE AND NO SERVE DECISION. Reads registered in
AMENDMENT 20: (1) PRIMARY J = E(MJ) minus E(M) where E(src) = src-q1
minus src-k0 paired; J >= +0.010 at 2 se JITTER IS THE MECHANISM, |J| <=
0.005 JITTER INNOCENT, else BETWEEN. (2) E(M), E(P) reproduce AMENDMENT
18. (3) MJ-k0 minus M-k0, jitter alone on the AR baseline.
"""
...
def main():
    ...
    jrng = np.random.default_rng(17000 + a.seed)
    jdur = mdur + jrng.normal(0.0, 0.02 * 1.0, size=len(mdur))
    jdur = np.log(np.clip(np.exp(jdur), 0.05, 4.0))
    B = len(geo)

    def cond_of(dur):
        return torch.tensor(np.stack([geo[:, 0], dur, geo[:, 1], geo[:, 2]], 1),
                            dtype=torch.float32)

    conds = {"P": cond_of(pdur), "M": cond_of(mdur), "MJ": cond_of(jdur)}
    print(f"  specs {B} of {a.n}, seed {a.seed}", flush=True)
    print(f"  log dur medians  P {np.median(pdur):+.4f}  M {np.median(mdur):+.4f}"
          f"  MJ {np.median(jdur):+.4f}  mean |MJ-M| {np.mean(np.abs(jdur-mdur)):.4f}",
          flush=True)
    ...
    for csrc in ("M", "MJ", "P"):
        for qarm in ("k0", "q1"):
            label = f"{csrc}-{qarm}"
            paths, info = generate(conds[csrc], QT[qarm], label)
