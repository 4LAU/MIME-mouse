"""w4_mserve, AMENDMENT 19 in step0_prereg.md. Read that first.

Serve candidate test. Per seed: one spec set from make_specs; TWO arms on
identical specs: k0 = protocol durations (esp._duration.sample, exactly
the confirm), no q; mq1 = matched durations (durmatch 64 NN matcher, rng
9000+seed) plus full q forcing event 0 (training/w4_firsthead_q.pt, temps
1,1,1). Confirm per batch torch seeding, contract scorer after shuffle.
Seeds 20 to 29, n 2000, batch 200.

READ 1 (PRIMARY, registered): paired mq1 minus k0, ten seeds; <= -0.008
at 2 paired se SERVE, |mean| <= 0.004 NO SERVE, else BETWEEN and no rerun
without a design change. READ 2: mq1 mean vs baseline 0.5881, support
only. One trajectory per spec, no selection.
"""
...
def main():
    ...
    for label, csrc, qarm in (("k0", "P", "k0"), ("mq1", "M", "q1")):
