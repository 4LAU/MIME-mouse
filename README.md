...
### The harder question: one trajectory, no selection

Everything above allows the pipeline to generate several candidate paths and keep one. Work since July 2026 asks a stricter question: what can the model do when it returns exactly one path per request, chosen before it is scored, and that path is required to start and end on the requested whole pixel? That is the product shape anything real would need, and selection is not available in it.

Two things make this harder than the 0.652 row above. There is no second chance at a bad draw. And forcing the path to land on the target pixel is itself expensive, because moving the points to hit the target damages the movement.

| Step | AUC | |
|---|---|---|
| Model alone, no correction | 0.654 | Lands on the requested pixel 0.4% of the time, so it is not a usable product |
| + the endpoint correction in service since July 20 | 0.721 | Lands 100% of the time. Enforcing arrival costs 0.067 |
| + a corrected endpoint correction, July 26 | 0.699 | Lands 100% of the time |
...
**The current headline, and what `generate.py` serves, is 0.5622** (standard error 0.0035 over ten seeds of 2000 requests each, one trajectory per request, no candidates and no selection, scored on the raw path as `generate.py` decodes it). The model is the autoregressive one above with one architectural change, a direction head that also reads the speed the model has just chosen (`training/event_ar_hm_mlp.pt`, the only change to the network that has ever moved this detector, 0.6340 to 0.6145 over five seeds). It is sampled at temperatures 0.95 for speed, 0.90 for direction and 1.00 for timing, the best point of a sweep, with four additions to the serving recipe, each tested against a pre-registered threshold before being adopted. The commanded duration of each request is drawn from real human movements of about the same length instead of from a fitted curve. The first event of each path comes from a small separate model trained only on how human movements begin, and that model's pause and direction heads carry a small two-layer network instead of a single linear layer, so the opening pause can depend jointly on the opening step, its direction and the geometry (`training/firsthead_q2.pt`). Those three were worth 0.010 and 0.011 on paired tests, and the served first event now reads at the level of a real human first event looked up by nearest neighbour. The fourth addition is in the decoder, not the model: every motion step is rounded to whole pixels before it is added to the path, the way a mouse reports movement. The research decoder behind the earlier records rounded only steps shorter than 2.5 px and let longer steps accumulate fractional positions that were rounded once at the end. Rounding every step moved all twenty two seeds tested, 0.0071 closer to human on average (standard error 0.0006) on the same token streams, and it is the reason the headline is quoted on the served decoder rather than the research one (the same token streams read 0.5686 through the old decoder, standard error 0.0015; `research/w4_snapdecode.json` holds both). The gain is specific to the model's token streams: a human's own recorded tokens read the same through either decoder, so what the rounding removes is something in how the model's steps accumulate, and the rest of the gap is in the token sequences themselves. Under the same detector, real recorded human movement from the training corpus reads about 0.5455 when built one way and 0.02 to 0.04 higher when built another (the detector's direction-change count is sensitive to how the path is assembled), so that floor is a rough guide and the paired comparisons above are the numbers to trust. Every number in this paragraph was measured on raw model output, which misses the target by a median of about 3 pixels. Making a served path end on its target pixel, by whole-pixel changes on its longest steps, costs 0.006 on average over the ten seeds (0.5622 raw against 0.5658 landed, standard error 0.0030); rotating and scaling the path instead, which is what a first version of this tool did, put every point off the pixel grid and cost 0.12. `verify_serve.py` prints the landing cost next to the raw score so it is never hidden inside the headline.
...
**Verify the served model, no GPU.** `python verify_serve.py --seed 20` regenerates the recorded seed 20 run of the current headline (2000 requests) through the code `generate.py` uses and scores it with `research/autoloop/scoring.py`, a random forest out-of-bag AUC against the human reference in `data/human_val_features_grpo.npy`. The record for that seed is 0.5653 raw and 0.5697 landed (`research/w4_snapdecode.json`; seed 21 is 0.5522 and 0.5506). It prints three numbers from one set of sampled token streams: the served paths as `generate.py` decodes them, the same rows after landing, and the same tokens decoded the way the earlier record was made (steps under 2.5 px rounded, the record's timestamp arithmetic), checked against that record in `research/w4_q2serve_s20.json`. On the machine the records were made on, all three return their logged values to four decimals, so the token streams are the record's. On other hardware the random draws differ, so the check passes within 0.03 rather than to the fourth decimal (a CPU only run of seed 20 on the recording machine read 0.024 under its record, seed 21 read 0.007 over). The registered research run itself is `research/w4_q2serve.py`; it also regenerates the earlier 0.5795 recipe on the same requests and matched its record (`research/w4_mserve_s*.json`) to four decimals on all ten seeds before the new number was read.
...
├── verify_serve.py                   # One-command check that generate.py reproduces the 0.5622 record
...
- **Training against the model's own output.** This has now partly worked and is the most concrete of the four. Because every per-step decision is already correct on recorded data and only free-running composition is wrong (see the single-trajectory section above), the objective has to see trajectories the model actually generated rather than trajectories a human generated. Matching the whole generated distribution of the eighteen measurements against the human one, with the gradient pushed back through sampling, moves the detector from 0.6301 to 0.5990. Two earlier attempts in this family failed and are worth knowing about: the GRPO pilot collapsed the model's variety, and a learned adversarial critic ran away from the generator faster than the generator could follow. A fixed distributional statistic avoids both. What has not worked is the cheaper version of the same idea, matching each measurement's average and spread one at a time, which returns nothing measurable. Repairing all eighteen marginals perfectly is worth about 0.030 and the trained model has already reached that point, so the room that remains is in the relationships between measurements rather than in any measurement on its own.
